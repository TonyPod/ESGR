...
class LLGAN(object):
    ...
    def classify(self, inputs):
        ...
        with self.graph.as_default():
            inputs = 2 * ((tf.cast(inputs, tf.float32) / 255.) - .5)
            _, classOutput = self.discriminator(inputs, reuse=True)
            result = tf.argmax(classOutput, 1)
    ...
    def train(self, data_X, data_y, test_X, test_y, category_idx):
        ...
        for iteration in range(self.iters):
            ...
            if (iteration+1) % 100 == 0:
                ...
                _test_accu = np.sum(self.classify(test_X) == np.argmax(test_y, 1)) / float(len(test_X))
