...
'''
Note: the accuracy curve of my implementation of LwF (Learning without forgetting) is not higher than that in the
released codes of iCaRL (https://github.com/srebuffi/iCaRL) and I don't know why so far. Therefore, in my paper
I report the result of the iCaRL's implementation of LwF (iCaRL name it "hb1" in the code)
I think my implementation of LwF in the class incremental learning setting is more neat.
...
flags.DEFINE_float('base_lr', 0.01, 'lenet: 0.01, nin: 0.1, resnet: 0.1 for softmax')
flags.DEFINE_float('weight_decay', 0.00001, '0.00001, resnet: 0.005->0.002')
...
flags.DEFINE_float('distillation_weight', 0., '')
flags.DEFINE_boolean('old_new_seperate', True, 'Is no_truncate is True, this one will have no effect')
...
def main(_):
    ...
    method_name += '' if FLAGS.temperature == 2. else '_%.1f' % FLAGS.temperature
    ...
    if FLAGS.use_momentum:
        opt_without_distillation = tf.train.MomentumOptimizer(learning_rate, FLAGS.momentum).minimize(loss_without_distillation, global_step=batch)
    else:
        opt_without_distillation = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss_without_distillation, global_step=batch)
    ...
    train_loss_without_distillation_summary = tf.summary.scalar('train_loss_without_distillation', loss_without_distillation)
    ...
    for category_idx in range(FLAGS.from_class_idx, FLAGS.to_class_idx + 1, FLAGS.nb_cl):
        ...
        if to_category_idx > 0 or not FLAGS.use_softmax:
            ...
            if with_distillation_val:
                if FLAGS.use_softmax:
                    train_y_old_truncated = sess.run(tf.nn.softmax(test_masked_logits),
                                                     feed_dict={batch_images: train_x,
                                                                mask_output: mask_output_val_prev})
                else:
                    train_y_old_truncated = sess.run(tf.nn.sigmoid(test_masked_logits),
                                                     feed_dict={batch_images: train_x,
                                                                mask_output: mask_output_val_prev})

            if FLAGS.no_truncate:
                train_y_new_truncated = train_one_hot_labels[train_indices, :]
            else:
                if FLAGS.old_new_seperate:
                    train_y_new_truncated = train_one_hot_labels[train_indices, category_idx:to_category_idx+1]
                else:
                    train_y_new_truncated = train_one_hot_labels[train_indices, :to_category_idx+1]
