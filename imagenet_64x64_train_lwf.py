...
flags.DEFINE_string("dataset", "imagenet_64x64", "The name of dataset")
...
flags.DEFINE_boolean('old_new_seperate', False, 'True and sigmoid=hybrid1; False and softmax=lwf(the advice from the author)')
...
def main(_):
    ...
    method_name = '_'.join(os.path.basename(__file__).split('.')[0].split('_')[3:])
    cls_func = '' if FLAGS.use_softmax else '_sigmoid'
    result_folder = os.path.join(FLAGS.result_dir, FLAGS.dataset + ('_flip' if FLAGS.flip else ''),
    ...
    for category_idx in range(FLAGS.from_class_idx, FLAGS.to_class_idx + 1, FLAGS.nb_cl):
        ...
        if FLAGS.no_truncate:
            ...
            mask_output_val[:] = False
            mask_output_val_prev[:] = False
            if FLAGS.old_new_seperate:
                mask_output_val[category_idx:to_category_idx+1] = True
